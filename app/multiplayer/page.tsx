...
import { createRoom, addPlayerToRoom, getRoomDetails } from "@/lib/supabase-service"
...
export default function MultiplayerPage() {
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      const result = await createRoom(playerId, username)

      if (result.error) {
        console.error("Oda oluşturma hatası:", result.error)
        setError(`Oda oluşturulurken bir hata oluştu: ${result.error}`)
        toast({
          title: "Hata",
          description: `Oda oluşturulurken bir hata oluştu: ${result.error}`,
          variant: "destructive",
        })
        setIsLoading(false)
        return
      ...
      if (!result.room) {
        setError("Oda oluşturulurken beklenmeyen bir hata oluştu")
        toast({
          title: "Hata",
          description: "Oda oluşturulurken beklenmeyen bir hata oluştu",
          variant: "destructive",
        })
        setIsLoading(false)
        return
      ...
      console.log("Oda oluşturuldu:", result.room)

      // Oda oluşturuldu, oyun sayfasına yönlendir
      router.push(
        `/multiplayer/room/${result.room.room_code}?name=${encodeURIComponent(username)}&id=${playerId}&host=true`,
      )
    } catch (error) {
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      ...
      const roomResult = await getRoomDetails(roomCode)

      if (roomResult.error || !roomResult.room) {
        console.error("Oda bulunamadı:", roomResult.error)
      ...
      // Odaya oyuncu ekle
      const playerResult = await addPlayerToRoom(roomCode, playerId, username)

      if (playerResult.error || !playerResult.player) {
        console.error("Oyuncu ekleme hatası:", playerResult.error)
        setError(`Odaya katılırken bir hata oluştu: ${playerResult.error}`)
        toast({
          title: "Hata",
          description: `Odaya katılırken bir hata oluştu: ${playerResult.error}`,
          variant: "destructive",
        })
        setIsLoading(false)
        return
      ...
      console.log("Odaya katıldı:", playerResult.player)
